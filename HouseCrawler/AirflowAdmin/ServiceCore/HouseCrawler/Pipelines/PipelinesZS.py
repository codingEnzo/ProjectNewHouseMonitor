...
try:
    from HouseCrawler.Items.ItemsZS import *
except Exception:
    import traceback
    traceback.print_exc()
...
class ZSPipeline(object):
    ...
    def check_item_exist(item):
        ...
        if isinstance(item, ProjectBaseItem):
            if q_object.filter(ProjectUUID=item['ProjectUUID']).latest(
                    field_name='CurTimeStamp'):
                exist_flag = True

        elif isinstance(item, BuildingInfoItem):
            if q_object.filter(BuildingUUID=item['BuildingUUID']).latest(
                    field_name='CurTimeStamp'):
                exist_flag = True

        elif isinstance(item, HouseInfoItem):
            if q_object.filter(HouseUUID=item['HouseUUID']).latest(
                    field_name='CurTimeStamp'):
                exist_flag = True
    ...
    def check_item_change(self, item):
        ...
        q_object = item.django_model.objects
        if isinstance(item, ProjectBaseItem):
            res_object = q_object.filter(ProjectUUID=item[
                'ProjectUUID']).latest(field_name='CurTimeStamp')
            for key in item:
                if not hasattr(res_object, key):
                    diff_flag = True
                    break
                if self.sum_value_hash(item.get(key)) != self.sum_value_hash(
            ...
            res_object = q_object.filter(BuildingUUID=item[
                'BuildingUUID']).latest(field_name='CurTimeStamp')
            for key in item:
                if not hasattr(res_object, key):
                    diff_flag = True
                    break
            ...
            for key in item:
                ...
                if self.sum_value_hash(item.get(key)) != self.sum_value_hash(
                    ...
                    break
            if diff_flag:
                item['HouseSaleStateLatest'] = getattr(res_object,
                                                       'HouseSaleState')
