...
class ProjectInfoHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectInfo':
            ...
            pinfo['ProjectName'] = (
            sel.xpath('//td[font[text()="项目名称"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectUUID'] = uuid.uuid3(uuid.NAMESPACE_DNS,
                                              pinfo['ProjectName'] + get_url_id(response.url))
            pinfo['ProjectCompany'] = (sel.xpath(
                '//td[font[text()="开 发 商"]]/following-sibling::td[1]/a/font/text()').extract_first() or '').strip()
            pinfo['ProjectAddress'] = (
            sel.xpath('//td[font[text()="项目地点"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectUsage'] = (
            sel.xpath('//td[font[text()="项目性质"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectArea'] = (
            sel.xpath('//td[font[text()="用地面积"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectBuildArea'] = (
            sel.xpath('//td[font[text()="总建筑面积"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectRongjiRatio'] = (
            sel.xpath('//td[font[text()="容积率（%）"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectLvdiRatio'] = (
            sel.xpath('//td[font[text()="绿化率（%）"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectInvestment'] = (
            sel.xpath('//td[font[text()="总投资（万元）"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectBuildDate'] = (
            sel.xpath('//td[font[text()="开工日期"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectBuildingCompany'] = (
            sel.xpath('//td[font[text()="施工单位"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectManageCompany'] = (
            sel.xpath('//td[font[text()="物业管理"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            pinfo['ProjectDesignCompany'] = (
            sel.xpath('//td[font[text()="设计单位"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
            result.append(pinfo)
        return result
...
class BuildingListHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectInfo':
            ...
            if b_info:
                b_info['BuildingPlanAreaCode'] = (
                sel.xpath('//td[b[text()="用地规划"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingRegName'] = (
                sel.xpath('//td[b[text()="预售证号"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingPlanEngCode'] = (
                sel.xpath('//td[b[text()="工程规划"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingBuildEngCode'] = (
                sel.xpath('//td[b[text()="工程施工"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingAreaCode'] = (
                sel.xpath('//td[b[text()="国土证号"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingZZHouseNum'] = (
                sel.xpath('//td[b[text()="住宅套数"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingFZZHouseNum'] = (
                sel.xpath('//td[b[text()="非住宅套数"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingSalingNum'] = (
                sel.xpath('//td[b[text()="现可售套数"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingSoldNum'] = (
                sel.xpath('//td[b[text()="已售套数"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingZZUnitPrice'] = (
                sel.xpath('//td[b[text()="住宅销售均价"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingFZZUnitPrice'] = (
                sel.xpath('//td[b[text()="非住宅销售均价"]]/following-sibling::td[1]/text()').extract_first() or '').strip()
                b_info['BuildingURL'] = urlparse.urljoin('http://www.jjzzfdc.com.cn/WebClient/ClientService/',
                                                         'proxp.aspx?key=WWW_LPB_001&params={sid}'.format(
                                                             sid=get_url_sid(response.url)))
                b_info['BuildingUUID'] = uuid.uuid3(uuid.NAMESPACE_DNS,
...
class HouseInfoHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        def get_project_unitprice(BuildingUUID, houseTime=str(datetime.datetime.now())):
            ...
            cur_info = BuildingInfoJiujiang.objects.filter(BuildingUUID=BuildingUUID,
                                                           CurTimeStamp__lt=time_upbound,
                                                           CurTimeStamp__gt=time_lowbound). \
                           latest('CurTimeStamp') or \
                       BuildingInfoJiujiang.objects.filter(BuildingUUID=BuildingUUID). \
                           latest('CurTimeStamp')
            if cur_info:
                try:
