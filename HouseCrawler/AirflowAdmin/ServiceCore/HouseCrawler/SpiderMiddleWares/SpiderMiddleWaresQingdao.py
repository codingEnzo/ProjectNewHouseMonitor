...
class ProjectBaseHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        def get_projectID(string):
            ...
            try:
                t = regex.search(
                    r'javascript:detailProjectInfo\("(?<page>.+)"\)', string)
                if t:
                    projectID = t.group(1)
            except:
                pass
        ...
        if curPage and curPage == 1:
            total_page = get_totla_page(response.xpath(
                '//a[contains(@onclick,"GoToPage")]/../text()').extract_first())
            ...
            tr_arr = response.xpath(
                '//a[contains(@href,"javascript:detailProjectInfo")]/../..')
            for tr in tr_arr:
...
class ProjectInfoHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.request.method == 'POST':
            ...
            projectInfo['ProjectName'] = response.xpath(
                '//td[@class = "bszn_title"]/text()').extract_first()
            ProjectUUID = uuid.uuid3(
                uuid.NAMESPACE_DNS, ProjectID + projectInfo['ProjectName'])
            ...
            for tr in tr_arr:
                ...
                presellInfo['EarliestOpeningDate'] = tr.xpath(
                    'td[3]/text()').extract_first()
                presellInfo['SellAddress'] = tr.xpath(
                    'td[4]/text()').extract_first()
                presellInfo['SellTel'] = tr.xpath(
                    'td[5]/text()').extract_first()
                presellInfo['TotalAmount'] = tr.xpath(
                    'td[6]/text()').extract_first()
                presellInfo['TotalArea'] = tr.xpath(
                    'td[7]/text()').extract_first()
                presellInfo['OnsoldAmount'] = tr.xpath(
                    'td[8]/text()').extract_first()
                presellInfo['OnsoldArea'] = tr.xpath(
                    'td[9]/text()').extract_first()
                presellInfo['SoldAmount'] = tr.xpath(
                    'td[10]/text()').extract_first()
                presellInfo['SoldArea'] = tr.xpath(
    ...
    def process_spider_output(self, response, result, spider):
        ...
        for tr in tr_arr:
            ...
            if href:
                ...
                buildingInfo['BuildingUUID'] = uuid.uuid3(
                    uuid.NAMESPACE_DNS, buildingInfo['BuildingURL'])
                buildingInfo['BuildingName'] = tr.xpath(
                    'td[1]/a/text()').extract_first()
                buildingInfo['BuildingReferencePrice'] = tr.xpath(
                    'td[2]/text()').extract_first()
                buildingInfo['BuildingFloatingRange'] = tr.xpath(
                    'td[3]/text()').extract_first()
                buildingInfo['OnsoldAmount'] = tr.xpath(
                    'td[4]/text()').extract_first()
                buildingInfo['BookingAmount'] = tr.xpath(
                    'td[5]/text()').extract_first()
                buildingInfo['TotalAmount'] = tr.xpath(
                    'td[6]/text()').extract_first()
...
class HouseListInfoHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        colors = {'#FFFF00': '已签约', '#FF0000': '已登记',
                  '#00C200': '可售', '#FF00FF': '已付定金', '#FFFFFF': '未纳入网上销售'}
        ...
        for tr in tr_arr:
            ...
            for colNum, td in enumerate(td_arr[1:]):
                ...
                if len(a_arr) > 0:
                    ...
                    try:
                        t = regex.search(
                            r'javascript:houseDetail\(\'(?<houseID>.+)\'\)', href_text)
                        houseID = t.group(1)
                    except:
                        pass
                else:
...
class HouseInfoHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        price = response.xpath(
            '//td[contains(text(),"参考价格")]/following-sibling::td[1]/text()').extract_first()
        if price:
