		...
		if response.meta.get('GetPage'):
			...
			for i in range(total_page):
				project_base_req = Request(
					url='http://housing.gzcc.gov.cn/fyxx/fdcxmxx/index.shtml' if i == 0 else 'http://housing.gzcc.gov.cn/fyxx/fdcxmxx/index_{curPage}.shtml'.format(
						curPage=i),
...
class ProjectInfoHandleMiddleware(object):
	def __init__(self, settings):
		self.settings = settings
		self.img_redis = Redis(host='120.25.213.19', port=6379, db=1)
...
class HouseInfoHandleMiddleware(object):
	...
	def process_spider_output(self, response, result, spider):
		...
		houseDetailItem['IsPrivateUse'] = response.xpath('//td[starts-with(text(),"是否自用")]/following-sibling::td[1]/text()').extract_first()
		...
		yucezongmianji_gif_list = response.xpath('//td[starts-with(text(),"预测总面积")]/following-sibling::td[1]').re(
			'/images/(\w+).gif')
		houseDetailItem['ForecastBuildingArea'] = imgToNumberText(NUMBER_IMG, yucezongmianji_dot,yucezongmianji_gif_list)
		...
		shicezongmianji_gif_list = response.xpath('//td[starts-with(text(),"实测总面积")]/following-sibling::td[1]').re(
			'/images/(\w+).gif')
		houseDetailItem['MeasuredBuildingArea'] = imgToNumberText(NUMBER_IMG,shicezongmianji_dot,shicezongmianji_gif_list)
		...
		yucetaoneimianji_gif_list = response.xpath('//td[starts-with(text(),"预测套内面积")]/following-sibling::td[1]').re(
			'/images/(\w+).gif')
		houseDetailItem['ForecastInsideOfBuildingArea'] = imgToNumberText(NUMBER_IMG,yucetaoneimianji_dot,yucetaoneimianji_gif_list)
		...
		shicetaoneimianji_gif_list = response.xpath(
			...
			'/images/(\w+).gif')
		houseDetailItem['MeasuredInsideOfBuildingArea'] = imgToNumberText(NUMBER_IMG,shicetaoneimianji_dot,shicetaoneimianji_gif_list)
		...
		yucegongtanmianji_gif_list = response.xpath('//td[starts-with(text(),"预测公摊面积")]/following-sibling::td[1]').re(
			'/images/(\w+).gif')
		houseDetailItem['ForecastPublicArea'] = imgToNumberText(NUMBER_IMG, yucegongtanmianji_dot,yucegongtanmianji_gif_list)
		...
		shicegongtanmianji_gif_list = response.xpath('//td[starts-with(text(),"实测公摊面积")]/following-sibling::td[1]').re(
			'/images/(\w+).gif')
		houseDetailItem['MeasuredSharedPublicArea'] = imgToNumberText(NUMBER_IMG, shicegongtanmianji_dot,shicegongtanmianji_gif_list)
