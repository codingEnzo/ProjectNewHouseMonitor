...
def get_url_id(strUrl):
    res_id = '0'
    match_res = re.search(r'ProjectId=([-\d]+)', str(strUrl))
    if match_res:
...
class ProjectBaseHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectBase':
            base_url = 'http://www.xjqfdc.cn/House/ListCanSell?'
            req_dict = {'page': '',
                        'Place': '',
                        'Project': ''}
            ...
            for p in project_list:
                ...
                p_name = p.xpath('./td[2]/a/text()').extract_first() or ''
                p_href = urlparse.urljoin(response.url,
                                            p.xpath('./td[2]/a/@href').extract_first() or '')
                p_address = p.xpath('./td[3]/text()').extract_first() or ''
                p_house_saling = p.xpath('./td[4]/text()').extract_first() or ''
                p_other_saling = p.xpath('./td[5]/text()').extract_first() or ''
                p_saling_area = p.xpath('./td[6]/text()').extract_first() or ''
                p_company = p.xpath('./td[1]/text()').extract_first() or ''
                pb = ProjectBaseItem()
                ...
                pb['ProjectHouseSalingNum'] = p_house_saling
                pb['ProjectOtherSalingNum'] = p_other_saling
                pb['ProjectSalingArea'] = p_saling_area
