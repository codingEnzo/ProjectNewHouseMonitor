...
class GetProjectPageBaseHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'GetProjectBase':
            pageresults = response.xpath("/html/body/table[1]/tr[2]/td/table/tr[33]/td").extract_first()
            if pageresults:
...
class GetProjectBaseHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectBase':
            results = response.xpath("/html/body/table[1]/tr[2]/td/table/tr").extract()
        ...
        if response.meta.get('PageType') == 'SubProject':
            ...
            try:
                ...
                project_com_name = re.search('"_blank">(.*?)</a>', results[4][1], re.S).group(1)
            ...
            if price_results_div2 and '元' in price_results_div2:
                ...
                price_results = re.findall(r'ivalign="center">(.*?)</div>', final_result, re.S)
                projectinfobaseitem['projectprice'] = price_results[0].replace("元", "")
                projectinfobaseitem['projectpricehouse'] = price_results[1].replace("元", "")
                projectinfobaseitem['projectpriceback'] = price_results[2].replace("元", "")
                projectinfobaseitem['projectpricehouseback'] = price_results[3].replace("元", "")
            # //*[@id="div1"]/table
            price_results_div1 = response.xpath('//*[@id="div1"]/table').extract_first()
            if price_results_div1:
                ...
                projectinfobaseitem['cansalehouseareas'] = int(cansalehouseareas)
                ...
                projectinfobaseitem['registeredhousenum'] = int(registeredhousenum)
                projectinfobaseitem['registeredhouseareas'] = int(registeredhouseareas)
                Approvalcardturl = re.search("Presell.asp(.*?)'", response.text, re.S).group(1)
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'openingunit':
            ...
            responsetext = response.body.decode('gbk', 'ignore').replace('\r', ''). \
                replace('\n', '').replace('\t', ''). \
                replace(' ', '').replace(' ', '')
            openingunitdetails = re.findall(r'indextabletxt(.*?)</TR>', responsetext)
            ...
            for openingunitdetail in openingunitdetails:
                if 'itemLine' in openingunitdetail:
                    ...
                    approvalitem['Totalnum'] = int(item_details[4].replace(",", ""))
                    approvalitem['Totalhousenum'] = int(item_details[5].replace(",", ""))
        ...
        if response.meta.get('PageType') == 'buildingBase':
            ...
            for openingunitpagedetail in openingunitpagedetails:
                ...
                if building_total_areas:
                    buildingitem['buildingtotalareas'] = float(building_total_areas)
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'HouseBase':
            ...
            for building_detail_result in building_detail_results:
                ...
                house_details = re.findall(r'<tdbgco(.*?)</td>', building_detail_result)
                recordhouse = []
        ...
        if response.meta.get('PageType') == 'HouseInfo':
            ...
            house_use_type = response.xpath('//*[@id="Table1"]/tr/td/table/tbody/tr[4]/td[2]/text()').extract_first()
            if house_use_type:
                houseitem['house_use_type'] = house_use_type
            house_layout = response.xpath('//*[@id="Table1"]/tr/td/table/tbody/tr[5]/td[2]/text()').extract_first()
            if house_layout:
                houseitem['house_layout'] = house_layout
            house_area_pr_tn = response.xpath('//*[@id="Table1"]/tr/td/table/tbody/tr[7]/td[2]/text()').extract_first()
            if house_area_pr_tn:
                houseitem['house_area_pr_tn'] = house_area_pr_tn
            house_area_pr_ft = response.xpath('//*[@id="Table1"]/tr/td/table/tbody/tr[8]/td[2]/text()').extract_first()
            if house_area_pr_ft:
                houseitem['house_area_pr_ft'] = house_area_pr_ft
            house_area_pr_dx = response.xpath('//*[@id="Table1"]/tr/td/table/tbody/tr[9]/td[2]/text()').extract_first()
            if house_area_pr_dx:
