...
def get_house_req(strHref, ProjectName, ProjectUUID, BuildingName, BuildingUUID):
    ...
    house_header = {'Accept': 'application/json, text/javascript, */*; q=0.01',
                    ...
                    'Cookie': 'XSRF-TOKEN=%3D;',
                    'Host': 'www.cszjw.net',
                    'X-Requested-With': 'XMLHttpRequest'}
    req_url = 'http://www.cszjw.net/geths'
    match_id = re.search(
        r"javascript:hsjajx\('(.+)',(.+),(.+)\)", str(strHref))
    if match_id:
        req_dict = {'ywzh': str(match_id.group(1)),
                    'n': match_id.group(3), 'i': match_id.group(2)}
        ...
        if response.meta.get('PageType') == 'ProjectBase':
            ...
            for p in project_list:
                ...
                pb['ProjectUUID'] = uuid.uuid3(
                    uuid.NAMESPACE_DNS, p_name + p_id)
                pb['ProjectRegUUID'] = uuid.uuid3(
                    uuid.NAMESPACE_DNS, p_regname + p_id)
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectInfo':
            ...
            pinfo['ProjectDistrict'] = sel.xpath(
                '//body/div/div/div[1]/table/tr[2]/td[2]/text()').extract_first() or ''
            ...
            pinfo['ProjectHouseNum'] = sel.xpath(
                '//body/div/div/div[1]/table/tr[6]/td[2]/text()').extract_first() or ''
            pinfo['ProjectBusLine'] = sel.xpath(
                '//body/div/div/div[1]/table/tr[6]/td[4]/text()').extract_first() or ''
...
class BuildingListHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectInfo':
            p_name = sel.xpath(
                '//body/div/div/div[1]/table/tr[1]/th/text()').extract_first() or ''
            p_id = get_url_id(response.url)
            building_list = sel.xpath(
                '//div[@class="hs_table"]/table/tr[not(@class) and td]')
            for b in building_list:
                b_info = BuildingInfoItem()
                b_info['ProjectUUID'] = uuid.uuid3(
                    uuid.NAMESPACE_DNS, p_name + p_id)
                b_info['ProjectName'] = p_name
                b_info['BuildingRegName'] = b.xpath(
                    './td[1]/text()').extract_first() or ''
                b_info['BuildingRegDate'] = b.xpath(
                    './td[3]/text()').extract_first() or ''
                b_info['BuildingName'] = b.xpath(
                ...
                b_info['BuildingAreaLicenceCode'] = b.xpath(
                    './td[5]/text()').extract_first() or ''
                b_info['BuildingEngPlanLicenceCode'] = b.xpath(
                    './td[6]/text()').extract_first() or ''
                b_info['BuildingAreaPlanLicenceCode'] = b.xpath(
                    './td[7]/text()').extract_first() or ''
                b_info['BuildingBuildLicenceCode'] = b.xpath(
                    './td[8]/text()').extract_first() or ''
                b_info['BuildingRegArea'] = b.xpath(
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectInfo':
            ...
            for building_info in building_list:
                p_name = sel.xpath(
                    '//body/div/div/div[1]/table/tr[1]/th/text()').extract_first() or ''
                b_name = building_info.xpath(
                    './td[2]/text()').extract_first() or ''
                breg_name = building_info.xpath(
                    './td[1]/text()').extract_first() or ''
            ...
            house_list = sel.xpath('//tr[not(th) and not(@id)]')
            if sel.xpath('//tr[@id and @style]').extract_first():
                result.append(get_house_req(sel.xpath('//tr[@id and @style]/td[@onclick]/@onclick').extract_first(),
                                            response.meta.get('ProjectName') or '', response.meta.get('ProjectUUID') or '',
                                            response.meta.get('BuildingName') or '', response.meta.get('BuildingUUID') or ''))
            for house in house_list:
                ...
                hinfo['HouseName'] = json.loads('{"foo":"%s"}' % house.xpath(
                    './td[1]/text()').extract_first() or '').get('foo')
                if hinfo['HouseName'] != 'None':
                    hinfo['HouseUUID'] = uuid.uuid3(uuid.NAMESPACE_DNS, hinfo[
                                                    'HouseName'] + str(hinfo['BuildingUUID']))
                    ...
                    hinfo['HouseUsage'] = json.loads('{"foo":"%s"}' % house.xpath(
                        './td[3]/text()').extract_first() or '').get('foo')
                    hinfo['HouseSaleState'] = json.loads('{"foo":"%s"}' % house.xpath(
                        './td[9]/text()').extract_first() or '').get('foo')
                    hinfo['HouseDType'] = json.loads('{"foo":"%s"}' % house.xpath(
                        './td[5]/text()').extract_first() or '').get('foo')
                    result.append(hinfo)
        return result
