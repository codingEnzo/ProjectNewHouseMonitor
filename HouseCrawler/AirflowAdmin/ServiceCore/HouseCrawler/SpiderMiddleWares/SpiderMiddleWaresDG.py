...
class ProjectBaseHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.request.method == 'GET':
            ...
            print('current area', Selector(response).xpath('//option[@selected="selected"]/text()').extract_first())
...
class HouseInfoHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'HouseInfo':
            ...
            hinfo['HouseRecordState'] = (houseinfodetail_sel.xpath('./tr[4]/td[2]/text()').extract_first() or '').strip()
            hinfo['HousePledgeState'] = (houseinfodetail_sel.xpath('./tr[5]/td[2]/text()').extract_first() or '').strip()
            hinfo['HouseAttachState'] = (houseinfodetail_sel.xpath('./tr[6]/td[2]/text()').extract_first() or '').strip()
            hinfo['HouseUsage'] = (houseinfodetail_sel.xpath('./tr[2]/td[2]/text()').extract_first() or '').strip()
            hinfo['HouseFloorCross'] = (houseinfodetail_sel.xpath('./tr[1]/td[4]/text()').extract_first() or '').strip()
