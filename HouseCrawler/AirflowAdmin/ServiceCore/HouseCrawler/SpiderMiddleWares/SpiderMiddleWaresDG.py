    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.request.method == 'GET':
            ...
            for town_info in base_sel.xpath('//select[@id="townName"]/option'):
                pb = ProjectBaseItem()
                pb['ProjectDistrict'] = (town_info.xpath(
                    './text()').extract_first() or '').replace('东莞市', '')
                req_dict['townName'] = town_info.xpath(
                    './@value').extract_first() or ''
            ...
            print('current area', Selector(response).xpath(
                '//option[@selected="selected"]/text()').extract_first())
            project_list = Selector(response).xpath('//tr[@class!="tHead"]')
            for p in project_list:
                p_name = p.xpath('./td[1]/a/text()').extract_first()
                p_href = urlparse.urljoin(response.url, p.xpath(
...
class ProjectInfoHandleMiddleware(object):
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectInfo':
            ...
            pinfo['SourceURL'] = response.url
            pinfo['ProjectName'] = response.meta.get('ProjectName')
            pinfo['ProjectUUID'] = response.meta.get('ProjectUUID')
            pinfo['ProjectOpenDate'] = (response.xpath(
                '//td[contains(text(),"开盘时间")]/following-sibling::td[1]/text()').extract_first() or '').strip()
            req = Request(url=iframe_src, meta={
                          'PageType': 'ProjectDetailInfo', 'CurURL': response.url, 'item': pinfo}, method='GET', dont_filter=True)
            result.append(req)
        elif response.meta.get('PageType') == 'ProjectDetailInfo':
            info_sel = Selector(response)
            pinfo = response.meta.get('item')
            pinfo['ProjectAlias'] = info_sel.xpath(
                '//span[@id="Projectname"]/text()').extract_first() or ''
            pinfo['ProjectAddress'] = info_sel.xpath(
                '//span[@id="Address"]/text()').extract_first() or ''
            pinfo['ProjectUsage'] = info_sel.xpath(
                '//span[@id="yongtu"]/text()').extract_first() or ''
            pinfo['ProjectArea'] = info_sel.xpath(
                '//span[@id="Totalarea"]/text()').extract_first() or ''
            pinfo['ProjectHouseNum'] = info_sel.xpath(
                '//span[@id="TotalRoom"]/text()').extract_first() or ''
            pinfo['ProjectCompany'] = info_sel.xpath(
                '//span[@id="Companyname"]/text()').extract_first() or ''
            pinfo['ProjectCorporation'] = info_sel.xpath(
                '//span[@id="MBR2"]/text()').extract_first() or ''
            pinfo['ProjectRegName'] = info_sel.xpath(
                '//span[@id="YsZheng"]/text()').extract_first() or ''
            pinfo['ProjectAreaPlanLicenseCode'] = info_sel.xpath(
                '//span[@id="JiansheZheng"]/text()').extract_first() or ''
            pinfo['ProjectPlanLicenseCode'] = info_sel.xpath(
                '//span[@id="JiansheZheng2"]/text()').extract_first() or ''
            pinfo['ProjectConstructionLicenseCode'] = info_sel.xpath(
                '//span[@id="JiansheZheng3"]/text()').extract_first() or ''
            pinfo['ProjectConstructionCompany'] = info_sel.xpath(
                '//span[@id="Shigong"]/text()').extract_first() or ''
            pinfo['ProjectAuthenticFlag'] = info_sel.xpath(
                '//span[@id="lm"]/text()').extract_first() or ''
            pinfo['ProjectSaleArea'] = info_sel.xpath(
                '//span[@id="KeArea"]/text()').extract_first() or '0.0'
            pinfo['ProjectSaleNum'] = info_sel.xpath(
                '//span[@id="KeRoom"]/text()').extract_first() or '0'
            pinfo['ProjectSaledArea'] = info_sel.xpath(
                '//span[@id="YiArea"]/text()').extract_first() or '0.0'
            pinfo['ProjectSaledNum'] = info_sel.xpath(
                '//span[@id="YiRoom"]/text()').extract_first() or '0'
            pinfo['ProjectUnavailableArea'] = info_sel.xpath(
                '//span[@id="NoArea"]/text()').extract_first() or '0.0'
            pinfo['ProjectUnavailableNum'] = info_sel.xpath(
                '//span[@id="NoRoom"]/text()').extract_first() or '0'
            result.append(pinfo)
        return result
    ...
    def process_spider_output(self, response, result, spider):
        ...
        if response.meta.get('PageType') == 'ProjectInfo':
            ...
            for building_info in building_base_sel.xpath('//table[@id="houseTable_1"]/tr[@class!="tHead"]'):
                ...
                b_info['ProjectUUID'] = response.meta.get('ProjectUUID')
                b_info['ProjectRegName'] = building_info.xpath(
                    './td[1]/a/text()').extract_first() or ''
                b_info['BuildingName'] = building_info.xpath(
                    './td[2]/a/text()').extract_first() or ''
                b_info['BuildingUUID'] = uuid.uuid3(uuid.NAMESPACE_DNS, p_name + get_url_id(
                    response.url) + b_info['ProjectRegName'] + b_info['BuildingName'])
                b_info['BuildingFloorNum'] = building_info.xpath(
                    './td[3]/a/text()').extract_first() or '0'
                b_info['BuildingHouseNum'] = building_info.xpath(
                    './td[4]/a/text()').extract_first() or '0'
                b_info['BuildingUsage'] = building_info.xpath(
                    './td[5]/a/text()').extract_first() or ''
                b_info['BuildingSaleArea'] = building_info.xpath(
                    './td[6]/a/text()').extract_first() or '0.0'
                b_info['BuildingURL'] = urlparse.urljoin(
                    response.url, building_info.xpath('./td[1]/a/@href').extract_first())
                b_info['SourceURL'] = response.url
        ...
        if response.meta.get('PageType') == 'HouseInfo':
            if (not response.meta.get('ProjectName')) or (not response.meta.get('BuildingName'))\
                ...
                if result:
                    return result
                return []
            floor_list = Selector(response).xpath(
                '//table[@id="roomTable"]/tr[position()>1]')
            for floor_item in floor_list:
                cur_floor = floor_item.xpath(
                    './td[1]/text()').extract_first() or '0'
                ...
                for house_item in house_list:
                    ...
                    hinfo['HouseSaleState'] = get_house_state(
                        house_item.xpath('./@class').extract_first())
            ...
            houseinfodetail_sel = Selector(response).xpath(
                '//div[@class="content"]/table')
            hinfo['HouseRecordState'] = (houseinfodetail_sel.xpath(
                './tr[4]/td[2]/text()').extract_first() or '').strip()
            hinfo['HousePledgeState'] = (houseinfodetail_sel.xpath(
                './tr[5]/td[2]/text()').extract_first() or '').strip()
            hinfo['HouseAttachState'] = (houseinfodetail_sel.xpath(
                './tr[6]/td[2]/text()').extract_first() or '').strip()
            hinfo['HouseUsage'] = (houseinfodetail_sel.xpath(
                './tr[2]/td[2]/text()').extract_first() or '').strip()
            hinfo['HouseFloorCross'] = (houseinfodetail_sel.xpath(
                './tr[1]/td[4]/text()').extract_first() or '').strip()
            hinfo['HouseBuildingArea'] = (houseinfodetail_sel.xpath(
                './tr[2]/td[4]/text()').extract_first() or '0.0').replace('平方米', '').strip()
            hinfo['HouseInnerArea'] = (houseinfodetail_sel.xpath(
                './tr[3]/td[4]/text()').extract_first() or '0.0').replace('平方米', '').strip()
            hinfo['HouseShareArea'] = (houseinfodetail_sel.xpath(
                './tr[4]/td[4]/text()').extract_first() or '0.0').replace('平方米', '').strip()
            hinfo['HouseUnitPrice'] = (houseinfodetail_sel.xpath(
                './tr[6]/td[4]/text()').extract_first() or '0.0').replace('元/平方米', '').strip()
            hinfo['HousePrice'] = (houseinfodetail_sel.xpath(
                './tr[5]/td[4]/text()').extract_first() or '0.0').replace('元', '').strip()
            hinfo['HousePriceFlag'] = False
            for key in ('HouseSaleState', 'HouseRecordState', 'HousePledgeState', 'HouseAttachState'):
                if hinfo.get(key) in (None, ""):
                    check_flag = False
                    break
            if check_flag:
                result.append(hinfo)
