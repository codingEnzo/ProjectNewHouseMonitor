...
class ProjectBaseWulumuqi(Document):
    ...
    PromotionName = fields.StringField(
        default=None, null=True, blank=True, max_length=50)  # 推广名
...
class PresellInfoWulumuqi(Document):
    ...
    LandUse = fields.StringField(default='', max_length=100)  # 物业类型/土地用途
    LssueDate = fields.StringField(
        default='', max_length=100, null=True)  # 预售证核发时间
    Applycorp = fields.StringField(
        default='', max_length=100, null=True)  # 预售证申领单位
    LssuingAuthority = fields.StringField(
        default='', max_length=100, null=True, blank=True)  # 发证机关
    ApprovalPresalePosition = fields.StringField(
        default='', max_length=100, null=True, blank=True)  # 批准预售位置

    Bank = fields.StringField(
        default='', max_length=100, null=True, blank=True)  # 资金监管银行
    BankAccount = fields.StringField(
        default='', max_length=100, null=True)  # 资金监管银行账号
    # 接口返回的字段
    ...
    OpeningDate = fields.StringField(
        default='', max_length=100, null=True, blank=True)  # 开盘时间
    OpeningPrice = fields.FloatField(
        default=0.00, null=True, blank=True)  # 开盘价格
    ApprovalProjectName = fields.StringField(
        default='', max_length=100, null=True, blank=True)  # 预售审批项目名称
